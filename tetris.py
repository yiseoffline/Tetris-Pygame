...
class ColorFactory(Factory):  # 컬러 팩토리 클래스  컬러 리스트를 가지고 create메소드 구현
    colors = [
        (0, 0, 0),
        (255, 85, 85),
        (100, 200, 115),
    ...
    @staticmethod
    def create(index):      ##컬러 만들기 
        return ColorFactory.colors[index]

class ShapeFactory(Factory):    # 쉐입 팩토리 클래스  쉐입 리스트를 가지고 create메소드 구현
    shapes = [
        [[1, 1, 1],
         [0, 1, 0]],
    ...
    @staticmethod
    def create():       ## 모양 만들기  랜덤으로 생성
        return ShapeFactory.shapes[rand(len(ShapeFactory.shapes))]
...
class TetrisApp:
    ...
    def draw_matrix(self, matrix, offset):
        ...
        for y, row in enumerate(matrix):
            for x, val in enumerate(row):
                if val:
                    pygame.draw.rect(self.screen,ColorFactory.create(val),pygame.Rect((off_x + x) * cell_size,(off_y + y) * cell_size,cell_size,cell_size), 0)
    ...
    def drop(self, manual):
        if not self.state.gameover and not self.state.paused:
            ...
            self.stone_y += 1
            if self.p.check_collision(self.board, self.stone, (self.stone_x, self.stone_y)):
                self.board = self.p.join_matrixes(self.board, self.stone, (self.stone_x, self.stone_y))
                self.new_stone()
                cleared_rows = 0
                while True:
...
class PlayingState(GameState):
    ...
    def render(self):
        pygame.draw.line(self.app.screen, (255, 255, 255),(self.app.rlim + 1, 0), (self.app.rlim + 1, self.app.height - 1))
